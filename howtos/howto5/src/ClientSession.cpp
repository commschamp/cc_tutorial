...
{
    Msg1 msg;
    assert(msg.transportField_version().value() == 5U); // Default version is one of the schema.
    assert(msg.field_f2().isMissing()); // F2 is missing when default constructed
    assert(msg.field_f3().doesExist()); // F3 exists when default constructed

    msg.field_f1().value() = 1111;
    msg.field_f2().field().value() = 1234;
