...
{
    Msg2 msg;
    msg.field_f1().value() = 0xdeadbeef;

    // Send the message with "CRC-16" checksum
    msg.transportField_checksumType().value() = ChecksumType::Crc16;

    assert(m_frame.length(msg) == 12U); // Verify total message length
    sendMessage(msg);
}
...
void ClientSession::sendMsg3()
{
    ...
    // Send the message with "CRC-32" checksum

    msg.transportField_checksumType().value() = ChecksumType::Crc32;

    assert(m_frame.length(msg) == 16U); // Verify total message length
    sendMessage(msg);
}
