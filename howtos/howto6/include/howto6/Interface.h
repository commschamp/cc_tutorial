// Generated by commsdsl2comms v8.0.0

/// @file
/// @brief Contains definition of <b>"Interface"</b> interface class.

#pragma once

#include <tuple>
#include "comms/Message.h"
#include "comms/options.h"
#include "howto6/InterfaceCommon.h"
#include "howto6/MsgId.h"
#include "howto6/field/ChecksumType.h"
#include "howto6/field/FieldBase.h"

namespace howto6
{

/// @brief Extra transport fields of @ref Interface interface class.
/// @see @ref Interface
/// @headerfile howto6/Interface.h
struct InterfaceFields
{
    /// @brief Definition of <b>"ChecksumType"</b> field.
    struct ChecksumType: public
        howto6::field::ChecksumType<
            howto6::options::DefaultOptions,
            comms::option::def::HasName
        >
    {
    };

    /// @brief All the fields bundled in std::tuple.
    using All = std::tuple<
        ChecksumType
    >;
};

/// @brief Definition of <b>"Interface"</b> common interface class.
/// .incCommon Interface for all the messages. @n
/// @tparam TOpt Interface definition options
/// @headerfile howto6/Interface.h
template <typename... TOpt>
class Interface : public
    comms::Message<
        TOpt...,
        comms::option::def::BigEndian,
        comms::option::def::MsgIdType<howto6::MsgId>,
        comms::option::def::ExtraTransportFields<InterfaceFields::All>
    >
{
    using Base =
        comms::Message<
            TOpt...,
            comms::option::def::BigEndian,
            comms::option::def::MsgIdType<howto6::MsgId>,
            comms::option::def::ExtraTransportFields<InterfaceFields::All>
        >;
public:
    /// @brief Allow access to extra transport fields.
    /// @details See definition of @b COMMS_MSG_TRANSPORT_FIELDS_NAMES macro
    ///     related to @b comms::Message class from COMMS library
    ///     for details.
    ///
    ///     The generated values, types and functions are:
    ///     @li @b TransportFieldIdx_checksumType index, @b TransportField_checksumType type
    ///         and @b transportField_checksumType() access fuction for @ref InterfaceFields::ChecksumType field.
    COMMS_MSG_TRANSPORT_FIELDS_NAMES(
        checksumType
    );
};

} // namespace howto6
