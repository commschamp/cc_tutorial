...
void ServerSession::handle(Message& msg)
{
    ...
    output.resize(m_serverToClientFrame.length(msg));

    // Serialize message into the buffer (including framing)
    // The serialization uses polymorphic write functionality.
    auto writeIter = &output[0];
    auto es = m_serverToClientFrame.write(msg, writeIter, output.size());
...
std::size_t ServerSession::processInputImpl(const std::uint8_t* buf, std::size_t bufLen)
{
    ...
    return comms::processAllWithDispatch(buf, bufLen, m_clientToServerFrame, *this);
}
