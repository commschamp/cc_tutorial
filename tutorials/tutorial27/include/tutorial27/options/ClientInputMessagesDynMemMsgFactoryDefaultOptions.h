// Generated by commsdsl2comms v8.1.0

/// @file
/// @brief Contains definition of protocol client input messages dynamic memory allocation message factory options.

#pragma once

#include "tutorial27/options/DefaultOptions.h"
#include "tutorial27/sub1/factory/ClientInputMessagesDynMemMsgFactory.h"
#include "tutorial27/sub2/factory/ClientInputMessagesDynMemMsgFactory.h"

namespace tutorial27
{

namespace options
{

/// @brief Provided client input messages dynamic memory allocation message factory options of the protocol.
/// @details Must be used as the outermost wrapper of the protocol options.
template <typename TBase = tutorial27::options::DefaultOptions>
struct ClientInputMessagesDynMemMsgFactoryDefaultOptionsT : public TBase
{
    /// @brief Alias to actual message factory class.
    /// @details Exposes the same template parameters as @b comms::MsgFactory.
    template <typename TInterface, typename TAllMessages, typename... TOptions>
    using sub1_MsgFactory =
        tutorial27::sub1::factory::ClientInputMessagesDynMemMsgFactory<TInterface, ClientInputMessagesDynMemMsgFactoryDefaultOptionsT<TBase> >;

    /// @brief Alias to actual message factory class.
    /// @details Exposes the same template parameters as @b comms::MsgFactory.
    template <typename TInterface, typename TAllMessages, typename... TOptions>
    using sub2_MsgFactory =
        tutorial27::sub2::factory::ClientInputMessagesDynMemMsgFactory<TInterface, ClientInputMessagesDynMemMsgFactoryDefaultOptionsT<TBase> >;

    /// @brief Extra options for namespace.
    struct sub1: public TBase::sub1
    {
        /// @brief Extra options for frames.
        struct frame : public TBase::sub1::frame
        {
            /// @brief Extra options for layers of
            ///     @ref tutorial27::sub1::frame::Frame frame.
            struct FrameLayers : public TBase::sub1::frame::FrameLayers
            {
                /// @brief Extra options for @ref
                ///     tutorial27::sub1::frame::FrameLayers::Id
                ///     layer.
                using Id =
                    std::tuple<
                        comms::option::app::MsgFactoryTempl<sub1_MsgFactory>,
                        typename TBase::sub1::frame::FrameLayers::Id
                    >;
            }; // struct FrameLayers
        }; // struct frame
    }; // struct sub1

    /// @brief Extra options for namespace.
    struct sub2: public TBase::sub2
    {
        /// @brief Extra options for frames.
        struct frame : public TBase::sub2::frame
        {
            /// @brief Extra options for layers of
            ///     @ref tutorial27::sub2::frame::Frame frame.
            struct FrameLayers : public TBase::sub2::frame::FrameLayers
            {
                /// @brief Extra options for @ref
                ///     tutorial27::sub2::frame::FrameLayers::Id
                ///     layer.
                using Id =
                    std::tuple<
                        comms::option::app::MsgFactoryTempl<sub2_MsgFactory>,
                        typename TBase::sub2::frame::FrameLayers::Id
                    >;
            }; // struct FrameLayers
        }; // struct frame
    }; // struct sub2
};

/// @brief Alias to @ref ClientInputMessagesDynMemMsgFactoryDefaultOptionsT with default template parameter.
using ClientInputMessagesDynMemMsgFactoryDefaultOptions = ClientInputMessagesDynMemMsgFactoryDefaultOptionsT<>;

} // namespace options

} // namespace tutorial27
